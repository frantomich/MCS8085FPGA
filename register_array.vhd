...
ENTITY register_array IS
	...
	PORT(
		...
		reg_pair_data_in: IN STD_LOGIC_VECTOR(15 DOWNTO 0);     --Barramento de entrada dupla (16 bits).
	...
	BEGIN
		...
		ELSE
			CASE reg_sel IS
				...
				WHEN OTHERS => reg_pair_data_out <= (OTHERS => '0');
			END CASE;
		END IF;
		...
		IF reg_clk'EVENT AND reg_clk = '1' THEN
			...
			ELSIF reg_wr_en = '1' THEN
				...
				ELSE
					CASE reg_sel IS
						WHEN "000" =>
							reg_b <= reg_pair_data_in(15 DOWNTO 8);
							reg_c <= reg_pair_data_in(7 DOWNTO 0);
						WHEN "001" => 
							reg_d <= reg_pair_data_in(15 DOWNTO 8);
							reg_e <= reg_pair_data_in(7 DOWNTO 0);
						WHEN "010" =>
							reg_h <= reg_pair_data_in(15 DOWNTO 8);
							reg_l <= reg_pair_data_in(7 DOWNTO 0);
						WHEN "011" =>
							reg_sp <= reg_pair_data_in;
						WHEN OTHERS => NULL;
					END CASE;
				END IF;
			END IF;
